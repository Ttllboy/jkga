<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="110px">
      <!--<el-form-item label="预警照片" prop="warnImg">-->
      <!--  <el-input-->
      ...
      <!--  </el-date-picker>-->
      <!--</el-form-item>-->
      <el-form-item label="预警位置" prop="warnPlace">
        <el-input
      ...
      <el-form-item label="预警类型" prop="warnType">
        <el-select v-model="queryParams.warnType" placeholder="请选择预警类型" clearable>
          <el-option
            v-for="dict in dict.type.yy_warn"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jkza:yyWarn:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="yyWarnList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="预警位置" align="center" prop="warnPlace" />
...
<!--      </el-table-column>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        <el-form-item label="预警照片" prop="warnImg">
          <el-input v-model="form.warnImg" placeholder="请输入预警照片" />
        </el-form-item>
        <el-form-item label="预警时间" prop="warnTime">
          <el-date-picker clearable
        ...
        <el-form-item label="预警位置" prop="warnPlace">
          <el-input v-model="form.warnPlace" placeholder="请输入预警位置" />
        </el-form-item>
        <el-form-item label="预警类型" prop="warnType">
...
<script>
import { listYyWarn, getYyWarn, delYyWarn, addYyWarn, updateYyWarn } from "@/api/jkza/yyWarn";

export default {
  name: "YyWarn",
  dicts: ['yj_check', 'yy_warn', 'data_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listYyWarn(this.queryParams).then(response => {
        this.yyWarnList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateYyWarn(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addYyWarn(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/yyWarn/export', {
        ...this.queryParams
      }, `yyWarn_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
