<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <!--<el-form-item label="考勤缺失时间" prop="checkTime">-->
      <!--  <el-date-picker clearable-->
      <!--    v-model="queryParams.checkTime"-->
      <!--    type="date"-->
      <!--    value-format="yyyy-MM-dd"-->
      <!--    placeholder="请选择考勤缺失时间">-->
      <!--  </el-date-picker>-->
      <!--</el-form-item>-->
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['jkza:checkLose:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jkza:checkLose:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jkza:checkLose:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="checkLoseList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="考勤缺失时间" align="center" prop="checkTime" width="180">
        <template slot-scope="scope">
...
<script>
import { listCheckLose, getCheckLose, delCheckLose, addCheckLose, updateCheckLose } from "@/api/jkza/checkLose";
import {listBuildingNames} from "@/api/jkza/synergy";
import BuildingNames from "@/views/components/buildingName";

export default {
  name: "CheckLose",
  dicts: ['worker_type', 'data_type'],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCheckLose(this.queryParams).then(response => {
        this.checkLoseList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCheckLose(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCheckLose(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除考勤缺失编号为"' + ids + '"的数据项？').then(function() {
        return delCheckLose(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/checkLose/export', {
        ...this.queryParams
      }, `checkLose_${new Date().getTime()}.xlsx`)
    },
    //改变queryParams所属工地ID
