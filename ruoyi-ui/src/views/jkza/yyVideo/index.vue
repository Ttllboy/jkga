  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="110px">
      <el-form-item label="所属医院" prop="hosId">
        ...
        <!--/>-->
        <el-select v-model="queryParams.hosId" placeholder="请选择所属医院" @change="changeHosId" clearable>
          <el-option
            v-for="item in yyNames"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jkza:yyVideo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jkza:yyVideo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="yyVideoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="所属医院" align="center" prop="hosId" >
        <template slot-scope="scope" v-if="scope.row.hosId">
          <yyName :yyNames="yyNames" :hosId="scope.row.hosId"></yyName>
          <!--<StreetNames :streetNames="yyNames" :streetId="scope.row.hosId"></StreetNames>-->
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        <el-form-item label="所属医院" prop="hosId">
          <!--<el-input v-model="form.hosId" placeholder="请输入所属医院" />-->
          <el-select v-model="form.hosId" placeholder="请选择所属医院" @change="changeFormHosId" clearable>
            <el-option
              v-for="item in yyNames"
              :key="item.value"
              :label="item.label"
...
<script>
import {listYyVideo, getYyVideo, delYyVideo, addYyVideo, updateYyVideo, listYyNames} from "@/api/jkza/yyVideo";
import yyName from "@/views/components/yyName";
export default {
  name: "YyVideo",
  components:{yyName},
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listYyVideo(this.queryParams).then(response => {
        this.yyVideoList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
      listYyNames().then(res => {
        this.yyNames = res;
        console.log(res);
      })
    },
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateYyVideo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addYyVideo(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/yyVideo/export', {
        ...this.queryParams
      }, `yyVideo_${new Date().getTime()}.xlsx`)
    },
    //改变queryParams所属所属医院
    changeHosId(hosId){
      this.queryParams.hosId =hosId;
    },
    //改变form所属街道ID
    changeFormHosId(hosId){
      this.form.hosId =hosId;
    },
  }
};
</script>
