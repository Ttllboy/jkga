...
<script>
import { listStreet, getStreet, delStreet, addStreet, updateStreet } from "@/api/jkza/street";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除街道信息编号为"' + ids + '"的数据项？').then(function() {
        return delStreet(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/street/export', {
        ...this.queryParams
      }, `street_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
