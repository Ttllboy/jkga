    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['jkza:registered:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['jkza:registered:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jkza:registered:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jkza:registered:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listRegistered, getRegistered, delRegistered, addRegistered, updateRegistered } from "@/api/jkza/registered";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getRegistered(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除登记信息编号为"' + ids + '"的数据项？').then(function() {
        return delRegistered(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/registered/export', {
        ...this.queryParams
      }, `registered_${new Date().getTime()}.xlsx`)
    },
    //改变queryParams所属工地ID
