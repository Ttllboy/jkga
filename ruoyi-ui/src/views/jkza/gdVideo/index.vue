<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="工地ID" prop="buildingSiteId">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jkza:gdVideo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jkza:gdVideo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="gdVideoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="ID" align="center" prop="id" />
      <el-table-column label="工地ID" align="center" prop="buildingSiteId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="工地ID" prop="buildingSiteId">
          <el-input v-model="form.buildingSiteId" placeholder="请输入工地ID" />
        </el-form-item>
        <el-form-item label="驾驶舱ID" prop="jscId">
...
<script>
import { listGdVideo, getGdVideo, delGdVideo, addGdVideo, updateGdVideo } from "@/api/jkza/gdVideo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGdVideo(this.queryParams).then(response => {
        this.gdVideoList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateGdVideo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGdVideo(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/gdVideo/export', {
        ...this.queryParams
      }, `gdVideo_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
