    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['jkza:workerCheck:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['jkza:workerCheck:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jkza:workerCheck:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jkza:workerCheck:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="workerCheckList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="考勤时间" align="center" prop="checkTime" width="180">
        <template slot-scope="scope">
...
<script>
import { listWorkerCheck, getWorkerCheck, delWorkerCheck, addWorkerCheck, updateWorkerCheck } from "@/api/jkza/workerCheck";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWorkerCheck(this.queryParams).then(response => {
        this.workerCheckList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getWorkerCheck(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWorkerCheck(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWorkerCheck(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除考勤信息编号为"' + ids + '"的数据项？').then(function() {
        return delWorkerCheck(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jkza/workerCheck/export', {
        ...this.queryParams
      }, `workerCheck_${new Date().getTime()}.xlsx`)
    },
    //改变queryParams所属工地ID
